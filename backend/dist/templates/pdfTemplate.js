...
exports.itineraryHtml = itineraryHtml;
function itineraryHtml(username, itineraries) {
    const itinerary = itineraries._doc;
    const daysWiseHtml = Array.isArray(itinerary.itinerary) ? itinerary.itinerary.map((dayPlan, index) => {
        ...
        const planHtml = dayPlan.plan.map((place) => {
            if (!place.placeName) {
                console.error("Invalid place data:", place);
                return "<div>No place name available</div>";
            }
            const gmapsUrl = `https://google.com/maps/search/?api=1&query=${place === null || place === void 0 ? void 0 : place.placeName}`;
            return `
                <div class="place">
    ...
    const hotelHtml = Array.isArray(itinerary.hotels) ? itinerary.hotels.map((hotel, index) => {
        if (!hotel.hotelName) {
            console.error("Invalid hotel data:", hotel);
            return "<div>No hotel name available</div>";
        }
        const gmapsUrl = `https://google.com/maps/search/?api=1&query=${hotel === null || hotel === void 0 ? void 0 : hotel.hotelName}`;
        return `
            <div class="hotel">
